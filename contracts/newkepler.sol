...
contract Kepler is KIP17Full("Kepler-452b", "K452"), KIP17Mintable, KIP17MetadataMintable, KIP17Burnable, KIP17Pausable, Ownable {
    using SafeMath for uint256;
    using Counters for Counters.Counter;
    ...
    function change(uint256 _tokenId, string memory _tokenURI) public onlyMinter {
        _setTokenURI(_tokenId, _tokenURI);
    }

    function transferBatch(address[] memory _to, uint256 _start, uint256 _end) public onlyMinter {
        for (uint256 i = _start; i <= _end; i += 1) {
            transferFrom(msg.sender, _to[i], i);
        }
    }

    function mintBatch(address[] memory _to, uint256 _start, uint256 _end) public onlyMinter {
        for (uint256 i = _start; i <= _end; i += 1) {
            mint(_to[i], i);
        }
    }
