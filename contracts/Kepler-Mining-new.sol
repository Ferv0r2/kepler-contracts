...
contract KeplerMining is Ownable {
    ...
    function checkPaid(address _account) internal view returns (bool) {
        return payers[_account];
    }
    ...
    function usePickaxe(address _account, uint256 _id, uint256 _destructCount) external {
        ...
        if (checkPaid(_account)) {
            return;
        }

        nft37.useItem(_account, toolIds[_id], _destructCount);

        payers[_account] = true;
    }

    modifier onlyPayer(address _account) {
        require(checkPaid(_account), "Not Paid");
        _;
    }
    ...
    function mining(uint256 _id, uint256 _mixId, uint256 _mixCount, uint256 _getBackCount) external {
        ...
        require(checkPaid(msg.sender), "Not Paid");
