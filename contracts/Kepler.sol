...
contract Kepler is KIP17Full, KIP17Mintable, KIP17MetadataMintable, KIP17Burnable, KIP17Pausable, Ownable {
    ...
    function singleChange(uint256 tokenId, string calldata tokenURI) external onlyMinter {
        _setTokenURI(tokenId, tokenURI);
    }

    function multiChange(uint256[] calldata ids, uint256[] calldata urls) external onlyMinter {
        uint256 length = ids.length;

        for(uint256 i=0; i<length; i++) {
            _setTokenURI(ids[i], EncodeLibrary.toString(urls[i]));
        }
    }

    function multiTransfer(address[] calldata tos, uint256[] calldata ids) external {
        uint256 length = ids.length;
    ...
    function multiMinter(uint256[] calldata ids, uint256[] calldata urls) external onlyMinter {
        uint256 length = ids.length;
        for (uint256 i = 0; i < length; i += 1) {
