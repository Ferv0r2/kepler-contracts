...
contract KeplerEnumerable is KIP13, KIP17, IKIP17Enumerable {
    // Mapping from owner to list of owned token IDs
    mapping(address => uint256[]) private _burnedTokens;

    // Mapping from token ID to index of the owner tokens list
    mapping(uint256 => uint256) private _burnedTokensIndex;

    // Array with all token ids, used for enumeration
    uint256[] private _allBurnedTokens;

    // Mapping from token id to position in the allTokens array
    mapping(uint256 => uint256) private _allBurnedTokensIndex;
     ...
     */
    function tokenOfOwnerByIndex(address owner, uint256 index) public view returns (uint256) {
        require(index < balanceOf(owner), "KeplerEnumerable: owner index out of bounds");
        return _burnedTokens[owner][index];
    }
     ...
     */
    function tokenByIndex(uint256 index) public view returns (uint256) {
        require(index < totalSupply(), "KeplerEnumerable: global index out of bounds");
        return _allBurnedTokens[index];
    }

    function _pay(address owner, uint256 tokenId) internal {

        _addTokenToOwnerEnumeration(owner, tokenId);
        _addTokenToAllTokensEnumeration(tokenId);
    }

    function _unPay(address owner, uint256 tokenId) internal {

        _removeTokenFromOwnerEnumeration(owner, tokenId);
        // Since tokenId will be deleted, we can clear its slot in _ownedTokensIndex to trigger a gas refund
        _burnedTokensIndex[tokenId] = 0;
     ...
     */
    function _addTokenToOwnerEnumeration(address to, uint256 tokenId) private {
        _burnedTokensIndex[tokenId] = _burnedTokens[to].length;
        _burnedTokens[to].push(tokenId);
    }
     ...
     */
    function _addTokenToAllTokensEnumeration(uint256 tokenId) private {
        _allBurnedTokensIndex[tokenId] = _allBurnedTokens.length;
    ...
    function _removeTokenFromOwnerEnumeration(address from, uint256 tokenId) private {
        ...
        uint256 lastTokenIndex = _burnedTokens[from].length.sub(1);
        uint256 tokenIndex = _burnedTokensIndex[tokenId];

        // When the token to delete is the last token, the swap operation is unnecessary
        if (tokenIndex != lastTokenIndex) {
            uint256 lastTokenId = _burnedTokens[from][lastTokenIndex];

            _burnedTokens[from][tokenIndex] = lastTokenId; // Move the last token to the slot of the to-delete token
            _burnedTokensIndex[lastTokenId] = tokenIndex; // Update the moved token's index
        }

        // This also deletes the contents at the last position of the array
        _burnedTokens[from].length--;
    ...
    function _removeTokenFromAllTokensEnumeration(uint256 tokenId) private {
        ...
        uint256 lastTokenIndex = _allBurnedTokens.length.sub(1);
        uint256 tokenIndex = _allBurnedTokensIndex[tokenId];

        // When the token to delete is the last token, the swap operation is unnecessary. However, since this occurs so
        // rarely (when the last minted token is burnt) that we still do the swap here to avoid the gas cost of adding
        // an 'if' statement (like in _removeTokenFromOwnerEnumeration)
        uint256 lastTokenId = _allBurnedTokens[lastTokenIndex];

        _allBurnedTokens[tokenIndex] = lastTokenId; // Move the last token to the slot of the to-delete token
        _allBurnedTokensIndex[lastTokenId] = tokenIndex; // Update the moved token's index

        // This also deletes the contents at the last position of the array
        _allBurnedTokens.length--;
        _allBurnedTokensIndex[tokenId] = 0;
    }
}
...
contract KeplerShop is Ownable, KeplerEnumerable {
    ...
    function setPotionPrice(uint256 _price) external onlyOwner {
        potionPrice = _price;
    }

    function setkluPrice(uint256 _price) external onlyOwner {
        kluPrice = _price;
    }
    ...
    function useKlu(address _account, uint256 _shopId, uint256 _count) external {        
        ...
        klu.transferFrom(msg.sender, feeTo, kluPrice);
    ...
    function useStone(address _account, uint256 _id) external {
        ...
        nft37.mint(pickaxeIds[_id], msg.sender, 1);        
    }

    function upPotion(address _account, uint256 _id) external {
        require(_id % 10 <= 4, "Not Origin Potion");
        require(nft37.balanceOf(msg.sender, _id) >= potionPrice, "Less Potion");     

        nft37.useItem(_account, _id, potionPrice);
        nft37.mint(_id - 10, msg.sender, 1);
    }

    function mixPotion(address _account, uint256 _id) external {
        require(_id % 10 <= 4, "Not Origin Potion");
        require(nft37.balanceOf(msg.sender, _id) >= 1, "Less Potion");

        if (_id % 10 == 4) {
            require(nft37.balanceOf(msg.sender, _id - 4) >= 1, "Less Potion");
            nft37.useItem(_account, _id, 1);
            nft37.useItem(_account, _id - 4, 1);
        } else {
            require(nft37.balanceOf(msg.sender, _id + 1) >= 1, "Less Potion");
            nft37.useItem(_account, _id, 1);
            nft37.useItem(_account, _id + 1, 1);
        }

        nft37.mint(_id + 5, msg.sender, 1);      
    }
    ...
    function useNFT(uint256[] calldata _tokenIds, uint256 _count) external {
        ...
        require(length == _count, "Not Matching");
    ...
    function cancelUse(address _owner, uint256 _tokenId) external onlyOwner{
        _unPay(_owner, _tokenId);
    }
}