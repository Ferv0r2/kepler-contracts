...
contract KeplerItems is KIP37, KIP37Burnable, KIP37Pausable, KIP37Mintable, Ownable {
    ...
    function change(string calldata uri) external onlyOwner {
        _setURI(uri);
    }
    
    function createwithURI(uint256 id, uint256 initialSupply, string calldata uri) external onlyOwner {
        _setURI(uri);
        create(id, initialSupply, "");
    }

    function massMint(uint256 id, address to, uint256 value) external onlyMinter {
        mint(id, to, value);
    }
...
contract KeplerStoneMinter is Ownable {
    ...
    KeplerItems public nft = KeplerItems(0x050ae51CFD1C40E86E768fCA9De80EB9128b97e9);
    ...
    //     nft.safeTransferFrom(feeTo, msg.sender, id, count, "");
    ...
    function mint(uint256 id, uint256 count) external {
        ...
        nft.massMint(id, msg.sender, count);
        uint256 price = mintPrice.mul(count);
