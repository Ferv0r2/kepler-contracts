...
contract Kepler is KIP17Full("Kepler-452b", "K452"), KIP17Mintable, KIP17MetadataMintable, KIP17Burnable, KIP17Pausable, Ownable {
    ...
    function changeSingle(uint256 tokenId, string memory tokenURI) public onlyMinter {
        _setTokenURI(tokenId, tokenURI);
    }
    ...
    function burnSingle(uint256 _tokenId) public onlyMinter {
        _burn(_tokenId);
        emit Klaytn17Burn(msg.sender, _tokenId);
    }

    function burnBatch(uint256[] memory _ids) public onlyMinter {
        uint256 length = _ids.length;
